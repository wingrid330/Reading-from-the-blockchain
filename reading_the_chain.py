...
def connect_to_eth():
    ...
    url = "https://mainnet.infura.io/v3/5459039e93f046638f0bfa0bc0a56313"  # FILL THIS IN
...
def connect_with_middleware(contract_json):
    ...
    # Inject middleware (required for BNB testnet's consensus mechanism)
...
def is_ordered_block(w3, block_num):
    ...
    def get_priority_fee(tx):
        if tx.get("type") == "0x2":
            return min(tx.get("maxPriorityFeePerGas", 0), tx.get("maxFeePerGas", 0) - base_fee)
        elif "gasPrice" in tx:
    ...
    if priority_fees == sorted(priority_fees, reverse=True):
        ordered = True
